<template>
  <div class="ranking-list">
    <h2>{{ $t('ranking.prescriptionTitle') }}</h2>
...
import { fetchPrescriptionRankingData } from '@/api/tcm/ranking.js';

export default {
  name: 'PrescriptionRankingList',
  setup() {
    const rankingData = ref([]);
    const loadRanking = async () => {
      rankingData.value = await fetchPrescriptionRankingData();
    };
